 ...
 * Optimized for clean, flat-shaded 3D mesh aesthetics.
 */
const Sobel = {
  /**
  ...
  extractPoints: function(imageData) {
    ...
    // Spacing constraints:
    // - Along object contours, place vertices sparsely (every 24 pixels) for clean, large flat shapes.
    // - In flat regions (sky, uniform walls), place vertices very sparsely (every 75 pixels) to keep polygon count low.
    const edgeSpacing = 24;
    const flatGridSize = 75;
    const silhouetteThreshold = 40; // Detects strong silhouettes
    ...
    // 3. Scan for contours and place points along outlines
    ...
    // 4. Fill flat/unstructured regions with sparse vertices
    ...
    const borderSpacing = 30; // Increased spacing along borders
